...
RSpec.feature "Authns", type: :feature, :js => true do
  ...
  feature "sign-up" do
    context "valid registration" do
      it "creates account and navigates away from signup page" do
        start_time = Time.now
        signup user_props
    ...
    context "rejected registration" do
      ...
      it "account not created and stays on page" do
        dup_user = FactoryGirl.attributes_for(:user, :email =>
          user_props[:email])
        signup dup_user, false # shoud get rejected by server

        # account not created
        expect(User.where(:email => user_props[:email], :name =>
          user_props[:name])).to exist
        expect(User.where(:email => dup_user[:email], :name =>
          dup_user[:name])).to_not exist
      ...
      it "displays error messages" do
        ...
        expect(page).to have_css("#signup-form > span.invalid", :text =>
          "Password confirmation doesn't match Password")
        expect(page).to have_css("#signup-form > span.invalid", :text =>
          "Password is too short")
        expect(page).to have_css("#signup-form > span.invalid", :text =>
          "Email has already been taken")
        expect(page).to have_css("#signup-email span.invalid", :text =>
          "has already been taken")
        expect(page).to have_css("#signup-password span.invalid", :text =>
          "is too short")
    ...
    context "invalid field" do
      ...
      it "bad email" do
        fillin_signup FactoryGirl.attributes_for(:user, :email => "hahaha")
      ...
      it "missing password" do
        ...
        expect(page).to have_css("input[name='signup-password'].ng-invalid" \
          "-required")
          expect(page).to have_css("input[name='signup-password_confirmation']"\
            ".ng-invalid-required")
      end
    end
  end
  ...
  feature "login" do
    ...
    context "valid user login" do
      ...
      it "can access authenticated resources" do
        checkme
    ...
    context "invalid login" do
      ...
      it "error message displayed and leaves user unauthenticated" do
  ...
  feature "logout" do
    ...
    it "closes form and removes user name" do
      login_criteria = ["#navbar-loginlabel", :text => "Login"]
      user_name_criteria = ["#navbar-loginlabel",
        :text => /#{user_props[:name]}/]
    ...
    it "can no longer access authenticated resources" do
      logout
      checkme
      within("div.checkme-user") do
        expect(page).to have_no_css("label", :text => /#{user_props[:name]}/)
        expect(page).to have_css("label", :text => /You need to sign in/)
      end
    end
  end
end
